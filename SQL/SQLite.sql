...
(
	...
	NAME TEXT NOT NULL,
	-- 创建时间
	CREATETIME DEFAULT (DATETIME(CURRENT_TIMESTAMP,'localtime')) NOT NULL,
	-- 用户id，使用自增列作为主键。
	USERID INTEGER NOT NULL,
	-- 父目录id
	PARENTID INTEGER,
	FOREIGN KEY (USERID) REFERENCES ORG_USER (ID),
	FOREIGN KEY (PARENTID) REFERENCES DOC_CATALOG (ID)
);
...
(
	...
	-- 带有格式信息的文档内容
	HTML TEXT,
	FOREIGN KEY (CATALOGID) REFERENCES DOC_CATALOG (ID),
	FOREIGN KEY (USERID) REFERENCES ORG_USER (ID)
);
